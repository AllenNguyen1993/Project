    ...
    current_time=current_time+del_t;
     ...
     for i=2:N_x+1 
        ...
        for j=2:N_y+1
            ...
            %u_num_newer(j+1,i)=D*(-2*sum_del*u_num_current(j+1,i)+del_y_squared*(u_num_current(j+1,i-1)+u_num_current(j+1,i+1))+del_x_squared*(u_num_current(j,i)+u_num_current(j+2,i)))+u_num_current(j+1,i);
            %u_num_newer(j+2,i)=D*(-2*sum_del*u_num_current(j+2,i)+del_y_squared*(u_num_current(j+2,i-1)+u_num_current(j+2,i+1))+del_x_squared*(u_num_current(j+1,i)+u_num_current(j+3,i)))+u_num_current(j+2,i);
            %u_num_newer(j+3,i)=D*(-2*sum_del*u_num_current(j+3,i)+del_y_squared*(u_num_current(j+3,i-1)+u_num_current(j+3,i+1))+del_x_squared*(u_num_current(j+2,i)+u_num_current(j+4,i)))+u_num_current(j+3,i);
            %u_num_newer(j+4,i)=D*(-2*sum_del*u_num_current(j+4,i)+del_y_squared*(u_num_current(j+4,i-1)+u_num_current(j+4,i+1))+del_x_squared*(u_num_current(j+3,i)+u_num_current(j+5,i)))+u_num_current(j+4,i);
        end 
